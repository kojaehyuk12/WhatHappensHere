...
app = Flask(__name__)
openai.api_key = "sk-proj-nsntiCNHFBspv0tMFzhQT3BlbkFJ1sdU6qVp8OvXa7jJhaU9"
...
knowledge_message = (
    ...
    "사용자가 질문하면, 이 목록에서 사용자 질문과 유사하거나 포함하는 prompt를 찾아 해당 completion을 그대로 답변해주세요.\n"
...
def generate_response(user_input, conversation_context):
    ...
    # GPT 응답을 맥락에 추가
    conversation_context.append({"role": "assistant", "content": chatbot_reply})
    return chatbot_reply, conversation_context
...
def chat():
    ...
    if not data or "user_input" not in data:
        return jsonify({"error": "user_input을 제공해주세요."}), 400
